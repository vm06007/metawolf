...
function assertArgumentCount(count, expectedCount, message) {
    ...
    assert$1(count >= expectedCount, "missing argument" + message, "MISSING_ARGUMENT", {
        count: count,
        expectedCount: expectedCount
    });
    assert$1(count <= expectedCount, "too many arguments" + message, "UNEXPECTED_ARGUMENT", {
        count: count,
        expectedCount: expectedCount
    });
}
...
function toQuantity(value) {
    let result = hexlify$1(isBytesLike(value) ? value : toBeArray(value)).substring(2);
...
function createGetUrl(options) {
    async function getUrl(req, _signal) {
        assert$1(_signal == null || !_signal.cancelled, "request cancelled before sending", "CANCELLED");
...
class FetchRequest {
    ...
    set body(body) {
        if (body == null) {
            ...
            this.#body = toUtf8Bytes$1(JSON.stringify(body));
    ...
    get headers() {
        ...
        if (this.#creds) {
            headers["authorization"] = `Basic ${encodeBase64(toUtf8Bytes$1(this.#creds))}`;
        }
        if (this.allowGzip) {
    ...
    toString() {
        return `<FetchRequest method=${JSON.stringify(this.method)} url=${JSON.stringify(this.url)} headers=${JSON.stringify(this.headers)} body=${this.#body ? hexlify$1(this.#body) : "null"}>`;
    }
    /**
...
class FetchResponse {
    ...
    #error;
    toString() {
        return `<FetchResponse status=${this.statusCode} body=${this.#body ? hexlify$1(this.#body) : "null"}>`;
    }
    /**
    ...
    get bodyText() {
        try {
            return (this.#body == null) ? "" : toUtf8String$1(this.#body);
        }
        catch (error) {
    ...
    assertOk() {
        ...
        try {
            if (this.#body) {
                responseBody = toUtf8String$1(this.#body);
            }
        }
        catch (e) { }
...
function unpercent(value) {
    return toUtf8Bytes$1(value.replace(/%([0-9a-f][0-9a-f])/gi, (all, code) => {
...
class FixedNumber {
    ...
    mulSignal(other) {
        ...
        assert$1((value % this.#tens) === BN_0$9, "precision lost during signalling mul", "NUMERIC_FAULT", {
            operation: "mulSignal", fault: "underflow", value: this
        });
        return this.#checkValue(value / this.#tens, "mulSignal");
    }
    #div(o, safeOp) {
        assert$1(o.#val !== BN_0$9, "division by zero", "NUMERIC_FAULT", {
     ...
     */
    divSignal(other) {
        assert$1(other.#val !== BN_0$9, "division by zero", "NUMERIC_FAULT", {
            operation: "div", fault: "divide-by-zero", value: this
        });
        this.#checkFormat(other);
        const value = (this.#val * this.#tens);
        assert$1((value % other.#val) === BN_0$9, "precision lost during signalling div", "NUMERIC_FAULT", {
     ...
     */
    isZero() { return (this.#val === BN_0$9); }
    /**
     *  Returns true if %%this%% is less than ``0``.
     */
    isNegative() { return (this.#val < BN_0$9); }
    /**
    ...
    static fromValue(_value, _decimals, _format) {
        ...
        if (delta > 0) {
            const tens = getTens(delta);
            assert$1((value % tens) === BN_0$9, "value loses precision for format", "NUMERIC_FAULT", {
...
function encodeRlp(object) {
    let result = "0x";
    for (const v of _encode$1(object)) {
        result += nibbles[v >> 4];
...
function throwError$1(name, error) {
...
class Result extends Array {
    ...
    getValue(name) {
        ...
        if (value instanceof Error) {
            throwError$1(`property ${JSON.stringify(name)}`, value.error);
        }
        return value;
    }
    /**
...
class Reader {
    ...
    #incrementBytesRead(count) {
        ...
        assert$1(this.#maxInflation < 1 || this.#bytesRead <= this.#maxInflation * this.dataLength, `compressed ABI data exceeds inflation ratio of ${this.#maxInflation} ( see: https:/\/github.com/ethers-io/ethers.js/issues/4537 )`, "BUFFER_OVERRUN", {
    ...
    readIndex() {
        return toNumber$1(this.readBytes(WordSize));
    }
}
...
class Signature {
    ...
    static from(sig) {
        ...
        if (typeof (sig) === "string") {
            ...
            if (bytes.length === 64) {
                const r = hexlify$1(bytes.slice(0, 32));
                const s = bytes.slice(32, 64);
                const v = (s[0] & 0x80) ? 28 : 27;
                s[0] &= 0x7f;
                return new Signature(_guard$1, r, hexlify$1(s), v);
            }
            if (bytes.length === 65) {
                const r = hexlify$1(bytes.slice(0, 32));
                const s = bytes.slice(32, 64);
...
class SigningKey {
    ...
    constructor(privateKey) {
        ...
        this.#privateKey = hexlify$1(privateKey);
    }
    /**
     *  The private key.
     */
    ...
    computeSharedSecret(other) {
        ...
        return hexlify$1(secp256k1.getSharedSecret(getBytesCopy(this.#privateKey), getBytes(pubKey), false));
    }
    /**
    ...
    static computePublicKey(key, compressed) {
        ...
        return hexlify$1(point.toRawBytes(compressed));
    }
    /**
...
class NumberCoder extends Coder {
    ...
    encode(writer, _value) {
        ...
        else if (value < BN_0$6 || value > mask(maxUintValue, this.size * 8)) {
            this._throwError("value out-of-bounds", _value);
        }
        return writer.writeValue(value);
    }
...
class StringCoder extends DynamicBytesCoder {
    ...
    encode(writer, _value) {
        return super.encode(writer, toUtf8Bytes$1(Typed.dereference(_value, "string")));
    }
    decode(reader) {
 ...
 */
function id(value) {
    return keccak256(toUtf8Bytes$1(value));
}
...
function ensNameSplit(name) {
    const bytes = toUtf8Bytes$1(ensNormalize(name));
    const comps = [];
    if (name.length === 0) {
        return comps;
    }
...
function dnsEncode(name, _maxLength) {
    ...
    return hexlify$1(concat(ensNameSplit(name).map((comp) => {
...
function _parseLegacy(data) {
    ...
    if (r === BN_0$5 && s === BN_0$5) {
        ...
        assertArgument(chainId !== BN_0$5 || (v === BN_27 || v === BN_28), "non-canonical legacy v", "v", fields[6]);
...
class Transaction {
    ...
    set data(value) { this.#data = hexlify$1(value); }
    /**
    ...
    set blobs(_blobs) {
        ...
        for (let i = 0; i < _blobs.length; i++) {
            const blob = _blobs[i];
            if (isBytesLike(blob)) {
                assert$1(this.#kzg, "adding a raw blob requires a KZG library", "UNSUPPORTED_OPERATION", {
                    operation: "set blobs()"
                });
                ...
                const proof = hexlify$1(this.#kzg.computeBlobKzgProof(data, commit));
                blobs.push({
                    data: hexlify$1(data),
...
const hexFalse = toBeHex(BN_0$4, 32);
...
function getBaseEncoder(type) {
    // intXX and uintXX
    {
        ...
        if (match) {
            ...
            const boundsLower = signed ? ((boundsUpper + BN_1) * BN__1) : BN_0$4;
        ...
        return {
            ...
            message: encoder.visit(value, (type, value) => {
                ...
                if (type.match(/^bytes(\d*)/)) {
                    return hexlify$1(getBytes(value));
                }
                // uint or int
                if (type.match(/^u?int/)) {
    ...
    encodeFilterTopics(fragment, values) {
        ...
        const encodeTopic = (param, value) => {
            if (param.type === "string") {
                ...
                return keccak256(hexlify$1(value));
            }
            ...
            while (nextScan <= blockNumber) {
                ...
                for (let i = 0; i < block.length; i++) {
                    ...
                    if (tx.from === this.from && tx.nonce === this.nonce) {
                        ...
                        else if (tx.data === "0x" && tx.from === tx.to && tx.value === BN_0$3) {
                            reason = "cancelled";
                        }
...
function buildWrappedFallback(contract) {
    const populateTransaction = async function (overrides) {
        ...
        const noValue = (getBigInt((tx.value || BN_0$2), "overrides.value") === BN_0$2);
                    ...
                    case "erc1155": {
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (bytes) {
                                    linkage.push({ type: "!metadata", value: hexlify$1(bytes) });
                                }
                                return { url: null, linkage };
                            }
                            return { url: null, linkage };
...
class AbstractProvider {
    ...
    async getCode(address, blockTag) {
        return hexlify$1(await this.#getAccountValue({ method: "getCode" }, address, blockTag));
    }
    async getStorage(address, _position, blockTag) {
        const position = getBigInt(_position, "position");
        return hexlify$1(await this.#getAccountValue({ method: "getStorage", position }, address, blockTag));
    }
    // Write
...
class JsonRpcSigner extends AbstractSigner {
    ...
    async signMessage(_message) {
        ...
        return await this.provider.send("personal_sign", [
            hexlify$1(message), this.address.toLowerCase()
        ]);
    }
    ...
    async _legacySignMessage(_message) {
        ...
        return await this.provider.send("eth_sign", [
            this.address.toLowerCase(), hexlify$1(message)
        ]);
    }
}
/**
...
async function stall(timeout) {
    return new Promise((resolve) => {
        setTimeout(() => { resolve(); }, timeout);
    });
}
function hexlify(value) {
    const hexArray = Array.from(new Uint8Array(value));
    return "0x" + hexArray.map((b) => b.toString(16).padStart(2, "0")).join("");
}
 ...
 *  CBOR
 *
 *  This is a heavily restricted sub-set of CBOR. This serves the
 *  necessary purposes for Firefly communication, but is also much
 *  smaller (saves 80kb!).
 *
 *  Uint8Array is used to represent binary strings instead of
 *  JavaScript "Binary Strings" which have a variety of
 *  browser-dependant quirks.
 *
 *  Restrictions:
 *    - numbers must be positive (may be relaxed in the future)
 *    - numbers must be 53-bit or less (otherwise IEEE-754 damages them)
 *    - Unsupported: undefined
 *    - Unsupported: float
 *    - Unsupported: semantic types
 *    - Unsupported: indefinite length
 */
function getBeBytes(value) {
    const bytes = [];
    while (value) {
...
const BN_0 = BigInt(0);
const BN_8 = BigInt(8);
function getBeValue(value) {
    let result = BN_0;
    for (let i = 0; i < value.length; i++) {
        result <<= BN_8;
        result |= BigInt(value[i]);
    }
    return result;
}
function getHeader(type, value) {
    if (value < 23) {
        return [(type << 5) | value];
    }
    const v = getBeBytes(value);
    while (v.length & (v.length - 1)) {
        v.unshift(0);
    }
    const count = ({ 1: 24, 2: 25, 4: 26, 8: 27 })[v.length];
    assert(count, "value out of range");
    v.unshift((type << 5) | count);
    return v;
}
function _encode(data) {
    // Uint8Array; treat as binary string
    if (data instanceof Uint8Array) {
        const bytes = Array.from(data);
        return getHeader(2, bytes.length).concat(bytes);
    }
    // Array
    if (Array.isArray(data)) {
        return getHeader(4, data.length).concat(...data.map(_encode));
    }
    // null
    if (data === null) {
        return [(7 << 5) | 22];
    }
    // undefined; don't support this
    //if (data === undefined) { return [ (7 << 5) | 23 ]; }
    switch (typeof (data)) {
        case "boolean":
            return [(7 << 5) | (data ? 21 : 20)];
        case "number":
            // Only support positive integer values for now
            assert(Number.isSafeInteger(data) && data >= 0, "unsupported numeric value");
            return getHeader(0, data);
        case "object": {
            const follow = [];
            for (const key in data) {
                assert(typeof (key) === "string", "unsupported key type", { key });
                follow.push(_encode(key).concat(_encode(data[key])));
            }
            return getHeader(5, follow.length).concat(...follow);
        }
        case "string": {
            const bytes = Array.from(toUtf8Bytes(data));
            return getHeader(3, bytes.length).concat(bytes);
        }
    }
    assert(0, `unsupported type`, { value: data });
}
function _decode(data) {
    ...
    if (type === 7) {
        switch (value) {
            case 20: return { value: false, consumed };
            case 21: return { value: true, consumed };
            case 22: return { value: null, consumed };
        }
    ...
    if (value <= 23) {
        ...
        throw new Error("invalid short count");
    }
    else {
        const length = 1 << (value - 24);
        const v = getBeValue(data.slice(1, 1 + length));
    ...
    switch (type) {
        ...
        case 2:
            consumed += value;
            return { value: new Uint8Array(data.slice(0, value)), consumed };
        case 3:
            consumed += value;
            return { value: toUtf8String(data.slice(0, value)), consumed };
        case 4: {
            const result = [];
            for (let i = 0; i < value; i++) {
                const v = _decode(data);
                consumed += v.consumed;
                data = data.slice(v.consumed);
                result.push(v.value);
            }
        ...
        case 5: {
            const result = {};
            for (let i = 0; i < value; i++) {
                const k = _decode(data);
                consumed += k.consumed;
                data = data.slice(k.consumed);
                const v = _decode(data);
                consumed += v.consumed;
                data = data.slice(v.consumed);
...
function encode(data) {
    return new Uint8Array(_encode(data));
}
...
const CMD_QUERY = 0x3;
const CMD_RESET = 0x2;
const CMD_START_MESSAGE = 0x6;
const CMD_CONTINUE_MESSAGE = 0x7;
...
function throwError(message, props) {
    if (props == null) {
        props = {};
    }
    const error = new Error(message);
...
function setSecret(firefly, value) {
    secret.set(firefly, value);
}
function getSecret(firefly) {
    const result = secret.get(firefly);
...
let nextMessageId = 1;
class Firefly {
    constructor(device, server, content, logger) {
        const readBuffer = [];
        const destroy = [];
        let serial = Promise.resolve();
        const replyMap = new Map();
        setSecret(this, {
            device, server, content, logger, readBuffer, destroy,
            replyMap, serial, onDisconnect: null, onMessage: null
        });
        const message = {
        ...
        const process = async () => {
            const checksum = new Uint8Array(await window.crypto.subtle.digest("SHA-256", message.data.slice(32, message.length)));
            for (let i = 0; i < 32; i++) {
                if (checksum[i] !== message.data[i]) {
                    console.log("checksum mismatch", checksum, message);
                    return;
                }
            }
            try {
                const value = decode(message.data.slice(32));
                ...
                const replier = replyMap.get(value ? value.id : -1);
                if (replier) {
                    if (value.result) {
                        replier.resolve(value.result);
                    }
                    else if (value.error) {
                        replier.reject(new Error(value.error.message));
                    }
                }
                else {
                    console.log("Missing replier:", value);
                }
        ...
        };
        {
            const listener = (event) => {
                ...
                if (status === STATUS_OK) {
                    // ACK
                    console.log("<<< ACK", {
                        cmd: data[1], data: hexlify(data.slice(2))
                    });
                    ...
                    switch (status) {
                        ...
                        case ERROR_BUFFER_OVERRUN:
                            console.log(`Error: Buffer Overrun`);
                            break;
                        case ERROR_BUSY:
                        ...
                        case ERROR_UNSUPPORTED_VERSION:
                            ...
                            break;
                        default:
                            console.log(`Error: Unknown Error (0x${status.toString(16)})`);
                            break;
                    }
                    readBuffer.push(data);
                }
                else if (status === CMD_QUERY) {
                    serial = serial.then(() => {
                        return content.writeValue(new Uint8Array([
                            STATUS_OK, CMD_QUERY, 0x01, 0, 0, 0, 0, 0xff, 0xff, 0xff, 0xff
                        ]));
                    });
                }
                else if (status === CMD_RESET) {
                    message.offset = 0;
                    message.length = 0;
                }
                else if (status === CMD_START_MESSAGE) {
                    message.length = (data[1] << 8) | data[2];
                    message.data.set(data.slice(3), message.offset);
                    message.offset += data.length - 3;
                    console.log({ message });
                    if (message.offset === message.length) {
                        process();
                    }
                }
                else if (status === CMD_CONTINUE_MESSAGE) {
                    if (((data[1] << 8) | data[2]) !== message.offset) {
                        console.log("out of sync", (data[1] << 8) | data[2], message.offset, data);
                        return;
                    }
                    message.data.set(data.slice(3), message.offset);
                    message.offset += data.length - 3;
            ...
            content.addEventListener('characteristicvaluechanged', listener);
            destroy.push(async () => {
                content.removeEventListener("characteristicvaluechanged", listener);
                try {
                    await content.stopNotifications();
                }
                catch (e) { }
            });
            content.startNotifications();
        }
        {
            const listener = (event) => {
                console.log("LOG", toUtf8String(event.target.value));
            };
            logger.addEventListener('characteristicvaluechanged', listener);
            destroy.push(async () => {
                logger.removeEventListener("characteristicvaluechanged", listener);
                logger.stopNotifications();
            });
            logger.startNotifications();
        }
        destroy.push(async () => { await server.disconnect(); });
        destroy.push(async () => {
            device.removeEventListener("gattserverdisconnected", ondisconnect);
        });
    }
    get onmessage() {
        return getSecret(this).onMessage;
    }
    set onmessage(value) {
        getSecret(this).onMessage = value;
    }
    get ondisconnect() {
        return getSecret(this).onDisconnect;
    }
    set ondisconnect(value) {
        getSecret(this).onDisconnect = value;
    }
    async destroy() {
        const secret = getSecret(this);
        if (!secret.destroyPromise) {
            const destroy = secret.destroy;
            secret.destroyPromise = (async () => {
                for (const callback of destroy) {
                    await callback();
                }
            })();
        }
        await secret.destroyPromise;
    }
    async _send(data) {
        console.log(">>>", data.length ? {
            command: data[0],
            data: hexlify(data.slice(1))
        } : null);
        const secret = getSecret(this);
        secret.serial = secret.serial.then(() => {
            return secret.content.writeValue(data);
        });
    }
    _read() {
        const { readBuffer } = getSecret(this);
        const result = new Uint8Array(readBuffer.reduce((a, b) => {
            return a + b.length;
        }, 0));
        let offset = 0;
        for (const buffer of readBuffer) {
    ...
    async _sendCommand(command, data) {
        ...
        if (command === CMD_QUERY) {
            const result = await this._readResult();
            console.log({ result });
    ...
    async _query() {
        ...
        const query = await this._sendCommand(CMD_QUERY, payload);
        this._queryCache = query;
        return query;
    }
    get serialNumber() {
        assert(this._queryCache, "query not called yet");
        return this._queryCache.serial;
    }
    get modelNumber() {
        assert(this._queryCache, "query not called yet");
        return this._queryCache.model;
    }
    get model() {
    ...
    async _reset() {
        return this._sendCommand(CMD_RESET);
    }
    async _chunkAndSendMessage(message) {
        // Prepend the checksum
        {
            const checksum = await window.crypto.subtle.digest("SHA-256", message);
            const checkedMessage = new Uint8Array(message.length + 32);
            checkedMessage.set(new Uint8Array(checksum), 0);
            checkedMessage.set(message, 32);
        ...
        while (offset < message.length) {
            const data = message.slice(offset, offset + 506);
            payloads.push(data);
            offset += data.length;
        }
        for (let i = 0; i < payloads.length; i++) {
            ...
            const offset = i * 506;
            const preamble = offset ? offset : message.length;
            const cmd = offset ? CMD_CONTINUE_MESSAGE : CMD_START_MESSAGE;
            let payload = new Uint8Array(data.length + 2);
            payload[0] = preamble >> 8;
            payload[1] = preamble & 0xff;
            payload.set(data, 2);
            const now = getTime();
            await this._sendCommand(cmd, payload);
            console.log("DT", getTime() - now);
        }
    }
    async sendMessage(method, params) {
        const message = { v: 1, method, id: nextMessageId++, params };
        console.log(">>>", message);
        const { replyMap } = getSecret(this);
        const promise = new Promise((resolve, reject) => {
            replyMap.set(message.id, { resolve, reject });
        });
        await this._chunkAndSendMessage(encode(message));
        return promise;
    }
    async forget() {
        return await getSecret(this).device.forget();
    }
    static async discover(forceRequest) {
        assert(("bluetooth" in navigator) && isBluetooth(navigator.bluetooth), `no Bluetooth API present`, {});
        const devices = (navigator.bluetooth.getDevices) ?
            (await navigator.bluetooth.getDevices()) : [];
        console.log({ devices });
        let device;
        if (devices.length && !forceRequest) {
            device = devices[0];
        }
        else {
        ...
        const firefly = new Firefly(device, server, content, logger);
        const { version, length } = await firefly._query();
        if (version !== 0x01) {
            throwError("ERROR_UNSUPPORTED_VERSION", { version });
        }
        if (length) {
            await firefly._reset();
        }
        return firefly;
    }
}
...
connect.addEventListener('click', async function(event) {
  const firefly = await Firefly.discover();
  console.log({ firefly });
  window.firefly = firefly;
  console.log(await firefly.sendMessage(new Uint8Array([
     185,   0,   1, 100, 116, 101, 115, 116, 24, 123
  ])));
});
*/

class FireflySigner extends AbstractSigner {
    firefly;
    provider;
    constructor(firefly, provider) {
        super();
    ...
    async getAddress() {
        if (!this._getAddress) {
            this._getAddress = (async () => {
                const result = await this.firefly.sendMessage("ffx_accounts", []);
                return getAddress(hexlify$1(result[0]));
            })();
        }
        return await this._getAddress;
    }
    connect(provider) {
        return new FireflySigner(this.firefly, provider);
    }
    async signMessage(message) {
        if (typeof (message) === "string") {
            message = toUtf8Bytes$1(message);
        }
        const result = await this.firefly.sendMessage("ffx_signMessage", { message });
        return Signature.from({
            r: hexlify$1(result.r),
            s: hexlify$1(result.s),
            v: result.v
        }).serialized;
    }
    ...
    async signTransaction(tx) {
        ...
        if (tx.from != null) {
            ...
            assertArgument(getAddress((tx.from)) === address, "transaction from address mismatch", "tx.from", tx.from);
            delete tx.from;
        }
        console.log({ tx });
        const params = {
            chainId: toBeArray(tx.chainId || 0),
            gasLimit: toBeArray(tx.gasLimit || 0),
            type: toBeArray(tx.type || 0),
            nonce: toBeArray(tx.nonce || 0),
            maxFeePerGas: toBeArray(tx.maxFeePerGas || 0),
            maxPriorityFeePerGas: toBeArray(tx.maxPriorityFeePerGas || 0),
            value: toBeArray(tx.value || 0),
            //from: getBytes(from),
            to: getBytes(to ? to : "0x"),
            data: getBytes(tx.data || "0x")
        };
        console.log({ params });
        const result = await this.firefly.sendMessage("ffx_signTransaction", params);
        console.log(result);
        const btx = Transaction.from(tx);
        btx.signature = Signature.from({
            r: hexlify$1(result.r),
            s: hexlify$1(result.s),
            v: result.v
        });
        console.log({ btx });
        console.log({ from: btx.from, data: btx.unsignedSerialized });
        return btx.serialized;
    }
}

export { Firefly, FireflySigner, JsonRpcProvider, parseEther };
